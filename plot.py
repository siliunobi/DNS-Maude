...
FOLDER_THEN_FOLDER_RESULTS_MEASUREMENTS = "/results/measurements/"

MESSAGE_FILE_EXISTS = "FILE exists :"
MESSAGE_FILE_DOES_NOT_EXIST__PASS = "The file doesn't exist. PASS "
...
def get_new_values_attack_model(attack, resolvers_folders):
    ...
    new_filename = "res_10labels_01nsdel.txt"
    ...
    if "slowdns" in attack.name_attack.lower():
        ...
        for folder in resolvers_folders:
            filename = attack_folder + folder + FOLDER_THEN_FOLDER_RESULTS_MEASUREMENTS + new_filename
            if not os.path.isfile(filename):
                print(MESSAGE_FILE_DOES_NOT_EXIST__PASS)
                continue
...
def get_values_attack_model_with_filename(attack, resolvers_folders, filename2):
    ...
    if "slowdns" in attack.name_attack.lower() or "delay" in attack.name_attack.lower():
        # use read_to_sec
        for folder in resolvers_folders:
            filename = attack_folder + folder + FOLDER_THEN_FOLDER_RESULTS_MEASUREMENTS + filename2 + ".txt"  # get_filename(attack_folder + folder, dname=dname)
            # print(filename)
            if not os.path.isfile(filename):
                print(MESSAGE_FILE_DOES_NOT_EXIST__PASS)
                continue
        ...
        for folder in resolvers_folders:
            filename = attack_folder + folder + FOLDER_THEN_FOLDER_RESULTS_MEASUREMENTS + filename2 + ".txt"
            # print(filename)
            if not os.path.isfile(filename):
                print(MESSAGE_FILE_DOES_NOT_EXIST__PASS)
                continue
...
def subplot_attack_v2(ax, title, add_labels_xaxis, list_list_values_model, list_list_values_attack, list_linestyle,
                      list_colors,
...
def main_subqueries_three_qmin():
    ...
    names_lines = ["Model BIND 9.18.4", "BIND 9.18.4", "Model PowerDNS 4.7.3", "PowerDNS 4.7.3", "Model Unbound 1.16.0",
                   "Unbound 1.16.0", ]
    ...
    for index, attack in enumerate(attacks):
        ...
        if type(attack) is CCV_QMIN:
            list_list_values_model = get_new_values_attack_model(attack, ["qmin_enabled/" + res for res in
                                                                          resolvers_folders])  # get_values_attack_model_with_filename(attack, ["qmin_enabled" + "/" + res for res in resolvers_folders], "res_10labels_01nsdel")
        else:
            list_list_values_model = get_values_attack_model(attack,
                                                             ["qmin_disabled" + "/" + res for res in resolvers_folders])

        # print("LIST OF VALUES MODEL FOR ATTACK : " + attack.name_attack)
        # print(list_list_values_model)

        if type(attack) is SubqueriesCCV:
            list_list_values_attack = get_new_values_j(name_folders[index],
                                                       ["resolver-" + res for res in resolver_name_j],
                                                       sorting_type="zone")
...
def main_delay():
    ...
    for index, attack in enumerate(attacks):
        ...
        if type(attack) is CCV_QMIN_Delay:
            list_list_values_model = get_values_attack_model_with_filename(attack, ["qmin_enabled/" + res for res in
                                                                                    resolvers_folders],
                                                                           "res_10labels_01nsdel")
        else:
            # Do not use QMIN for this attack
            list_list_values_model = get_values_attack_model_with_filename(attack, ["qmin_disabled/" + res for res in
                                                                                    resolvers_folders],
                                                                           "res_00labels_01nsdel")
