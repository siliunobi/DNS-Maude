...
Note: a `requirements.txt` is also present in the root folder, in case someone wants to run the plotting file `plot.py` directly in the folder.
...
More precise steps that this script is fulfilling are further explained below.
...
1. Installing Go (If a Go version is already installed, it won't do anything). NOTE: the latest Go version the testbed was run with is 1.20.5, there is no guarantee the project can run correctly with other versions.
...
So we can extract the results of scripts being executed inside the container into a folder that we created. This is useful to easily and quickly inspect the output of our functions.
...
So far the only operating system that can entirely run our setup and the experiments is macOS 10.15. We tried other strategies to port the projects to other OSes such as dind ("docker-in-docker"), a docker that will create dockers for Maude and the testbed, or
